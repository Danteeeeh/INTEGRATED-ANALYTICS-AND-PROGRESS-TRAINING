...
@section('page-title-bar')
    <div class="page-title-bar">
        ...
        <div class="page-actions">
            <a href="{{ route('instructor.classes.calendar.index', $class) }}" class="btn-modal-cancel" style="padding:10px 18px;border-radius:8px;font-size:.85rem;font-weight:600;text-decoration:none;">
...
<section class="cal-create-hero">
    ...
    <div class="cal-create-copy">
        ...
        <h3>New Event — {{ $class->code }}</h3>
        <p>{{ $class->course->name ?? $class->course->title ?? 'Class calendar' }} · Magdagdag ng importanteng event</p>
    </div>
    <span class="cal-class-badge"><i class="fa-solid fa-school"></i> {{ $class->code }}</span>
</section>

<div class="cal-card">
    <div class="cal-card-head"><h3><i class="fa-solid fa-calendar-plus"></i> Event Details</h3></div>
    <form method="POST" action="{{ route('instructor.classes.calendar.store', $class) }}" id="calendarForm">
        @csrf
        <div class="cal-section">
            <div class="cal-grid">
                <div class="cal-field full">
                    <label>Event Title <span class="req">*</span></label>
                    <input type="text" name="title" value="{{ old('title') }}" required maxlength="255" placeholder="e.g. Midterm Exam">
                ...
                <div class="cal-field">
                    ...
                    @error('event_type')<span class="error-message">{{ $message }}</span>@enderror
                </div>
                <div class="cal-field">
                    <label>Visibility <span class="req">*</span></label>
                    <select name="visibility" required>
                        @foreach($visibilityOptions as $key => $label)
                            <option value="{{ $key }}" {{ old('visibility', 'class') === $key ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                ...
                <div class="cal-field">
                    ...
                    <input type="text" name="location" value="{{ old('location') }}" maxlength="255" placeholder="Room / online link">
                    @error('location')<span class="error-message">{{ $message }}</span>@enderror
                </div>
                <div class="cal-field" style="justify-content:center;">
                    <label class="cal-check"><input type="checkbox" name="is_all_day" value="1" {{ old('is_all_day') ? 'checked' : '' }}> All-day event</label>
                </div>
                <div class="cal-field full">
                    <label>Description</label>
                    <textarea name="description" rows="4" placeholder="Optional details para sa event...">{{ old('description') }}</textarea>
                    @error('description')<span class="error-message">{{ $message }}</span>@enderror
                </div>
            </div>
        </div>
        <div class="cal-footer">
            <a href="{{ route('instructor.classes.calendar.index', $class) }}" class="cal-btn cal-btn-cancel"><i class="fa-solid fa-xmark"></i> Cancel</a>
