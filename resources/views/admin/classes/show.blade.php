...
@section('page-title-bar')
    <div class="page-title-bar">
        <h2 class="page-title"><i class="fa-solid fa-chalkboard-user"></i> {{ $class->name }}</h2>
        <div class="page-actions">
            <a href="{{ route('admin.classes.edit', $class) }}" class="btn btn-primary"><i class="fa-solid fa-pen-to-square"></i> Edit class</a>
            <a href="{{ route('admin.classes.index') }}" class="btn btn-secondary"><i class="fa-solid fa-arrow-left"></i> Back</a>
        </div>
    </div>
@endsection
...
@section('content')
    ...
    <div class="class-details">
        <section class="detail-hero">
            ...
            <div>
                <h3>{{ $class->name }}</h3>
                <p>{{ $courseTitle }} · {{ $class->academicPeriod?->name ?? 'No academic period' }}</p>
            </div>
            <span class="detail-code">{{ $class->code }}</span>
        </section>

        <div class="detail-grid">
            <div class="metric"><span class="metric-label">Enrolled</span><span class="metric-value">{{ $enrolledCount }}{{ $capacity ? ' / '.$capacity : '' }}</span></div>
            <div class="metric"><span class="metric-label">Available seats</span><span class="metric-value">{{ $availableSeats ?? '—' }}</span></div>
            <div class="metric"><span class="metric-label">Instructor</span><span class="metric-value">{{ $class->instructor?->full_name ?? 'Unassigned' }}</span></div>
            <div class="metric"><span class="metric-label">Status</span><span class="metric-value"><span class="status-chip status-{{ $class->status }}"><i class="fa-solid fa-circle"></i> {{ ucfirst($class->status) }}</span></span></div>
        </div>
        ...
        <section class="detail-card">
            ...
            <div class="detail-card-body">
                <dl class="meta-grid">
                    <div class="meta-row"><dt>Course</dt><dd>{{ $courseTitle }}</dd></div>
                    <div class="meta-row"><dt>Academic period</dt><dd>{{ $class->academicPeriod?->code ?? '—' }}</dd></div>
                    <div class="meta-row"><dt>Instructor email</dt><dd>{{ $class->instructor?->email ?? '—' }}</dd></div>
                    <div class="meta-row"><dt>Schedule</dt><dd>{{ $class->schedule ?: 'Not specified' }}</dd></div>
                    <div class="meta-row"><dt>Room / location</dt><dd>{{ $class->room ?: 'Not specified' }}</dd></div>
                    <div class="meta-row"><dt>Enrollment capacity</dt><dd>{{ $capacity ?? 'Unlimited' }}</dd></div>
                </dl>
        ...
        <section class="detail-card">
            <div class="detail-card-head"><h3><i class="fa-solid fa-users"></i> Enrolled students</h3><span class="status-chip status-active">{{ $class->enrollments->count() }} records</span></div>
                ...
                <table>
                    ...
                    @forelse($class->enrollments as $enrollment)
                        ...
                        <tr>
                            ...
                            <td><span class="status-chip {{ $statusClass }}">{{ ucfirst($enrollment->status) }}</span></td>
