...
<div class="user-page">
    <x-user-page-header
        ...
        subtitle="Create a reusable set of questions."
        icon="fa-database"
    >
        <x-slot name="actions">
    ...
    <div class="user-panel">
        ...
        <div class="user-panel-body">
            <form action="{{ route('admin.question_banks.store') }}" method="POST">
                @csrf

                <div class="form-grid">
                    <div class="form-field full">
                        <label>Title <span class="required">*</span></label>
                        <input type="text" name="title" value="{{ old('title') }}" required placeholder="e.g. Midterm Biology Questions">
                        <span class="field-error">{{ $errors->first('title') }}</span>
                    </div>

                    <div class="form-field">
                        <label>Code</label>
                        <input type="text" name="code" value="{{ old('code') }}" placeholder="e.g. BIO-MID" maxlength="50">
                        <span class="field-error">{{ $errors->first('code') }}</span>
                    </div>

                    <div class="form-field">
                        <label>Category</label>
                        <input type="text" name="category" value="{{ old('category') }}" placeholder="e.g. Biology" maxlength="100">
                    ...
                    <div class="form-field full">
                        ...
                        <textarea name="description" rows="4" placeholder="Bank description">{{ old('description') }}</textarea>
                        <span class="field-error">{{ $errors->first('description') }}</span>
                    </div>
                </div>

                <div class="form-actions user-actions">
                    <a href="{{ route('admin.question_banks.index') }}" class="btn btn-secondary">
