...
@section('content')
    <div class="form-card">
        ...
        <div style="padding: 14px 0; display: flex; gap: 12px; flex-wrap: wrap;">
            <a href="{{ route('admin.virtual_classes.edit', $virtualClass) }}" style="padding: 9px 20px; background: linear-gradient(135deg, #2563eb 0%, #1d4ed8 100%); color: #fff; border: none; border-radius: 8px; font-size: 0.85rem; font-weight: 600; cursor: pointer; text-decoration: none; display: inline-flex; align-items: center; gap: 6px; transition: opacity 0.2s;">
                <i class="fa-solid fa-pen-to-square"></i> Edit Class
            </a>
            @if($virtualClass->meeting_url)
                <a href="{{ $virtualClass->meeting_url }}" target="_blank" style="padding: 9px 20px; background: linear-gradient(135deg, #059669 0%, #047857 100%); color: #fff; border: none; border-radius: 8px; font-size: 0.85rem; font-weight: 600; cursor: pointer; text-decoration: none; display: inline-flex; align-items: center; gap: 6px; transition: opacity 0.2s;">
                    <i class="fa-solid fa-arrow-up-right-from-square"></i> Open Meeting
                </a>
            @endif
            <form method="POST" action="{{ route('admin.virtual_classes.destroy', $virtualClass) }}" style="display:inline;" onsubmit="return confirm('Are you sure you want to delete this virtual class?');">
                @csrf
                @method('DELETE')
        ...
        @endphp
        <div class="modal-row"><span>Provider:</span><span><span style="padding: 3px 10px; border-radius: 20px; font-size: 0.78rem; font-weight: 600; {{ $providerBadgeStyle }}">{{ $providerLabel }}</span></span></div>
        <div class="modal-row"><span>Meeting URL:</span><span>
            @if($virtualClass->meeting_url)
                <a href="{{ $virtualClass->meeting_url }}" target="_blank" style="color: #2563eb; text-decoration: none;">{{ $virtualClass->meeting_url }} <i class="fa-solid fa-arrow-up-right-from-square" style="font-size: 0.75rem;"></i></a>
            @else
                Not provided
            @endif
        </span></div>
        ...
        @endphp
        <div class="modal-row"><span>Status:</span><span><span style="padding: 3px 10px; border-radius: 20px; font-size: 0.78rem; font-weight: 600; {{ $statusBadgeStyle }}">{{ ucfirst($virtualClass->status) }}</span></span></div>
    </div>
                    ...
                    @foreach($virtualClass->attendees as $attendee)
                        <tr>
                            ...
                            <td>
                                ...
                                @endphp
                                <span style="padding: 3px 10px; border-radius: 20px; font-size: 0.72rem; font-weight: 600; {{ $attBadgeStyle }}">
                                    {{ ucfirst($attendee->attendance_status ?? 'registered') }}
                                </span>
                            </td>
                        </tr>
                    @endforeach
    ...
    <div style="margin: 0 24px 24px;">
        <a href="{{ route('admin.virtual_classes.index') }}" class="btn-modal-cancel" style="display: inline-flex; align-items: center; justify-content: center; padding: 11px 32px; border-radius: 8px; font-size: 0.88rem; font-weight: 600; cursor: pointer; transition: background 0.2s; text-decoration: none;">
