...
<div class="user-page enrollment-create-page">
    <x-user-page-header
        ...
        subtitle="Place an active student into a class and start tracking their learning progress."
        icon="fa-user-plus"
    >
        <x-slot name="actions">
    ...
    <div class="enrollment-create-layout">
        ...
        <div class="user-panel enrollment-create-panel">
            ...
            <form action="{{ route('admin.enrollments.store') }}" method="POST" id="enrollmentCreateForm" class="enrollment-create-form">
                @csrf
                ...
                <section class="enrollment-create-section" aria-labelledby="create-student-heading">
                    ...
                    <div class="enrollment-create-field @error('student_id') has-error @enderror">
                        <label for="student_id">Student <span aria-hidden="true">*</span></label>
                        <select id="student_id" name="student_id" required aria-describedby="student-help student-error" @error('student_id') aria-invalid="true" @enderror>
                            <option value="">Select a student</option>
                            @foreach($students as $student)
                                <option value="{{ $student->id }}" data-email="{{ $student->email }}" data-identifier="{{ $student->identifier ?? '' }}" @selected((string) old('student_id') === (string) $student->id)>{{ $student->full_name }} — {{ $student->email }}</option>
                            @endforeach
                        </select>
                        ...
                        @error('student_id')<span id="student-error" class="enrollment-create-error" role="alert">{{ $message }}</span>@enderror
                    </div>
                </section>
                ...
                <section class="enrollment-create-section" aria-labelledby="create-details-heading">
                    ...
                    <div class="enrollment-create-field-grid">
                        <div class="enrollment-create-field @error('status') has-error @enderror">
                            ...
                            @error('status')<span id="status-error" class="enrollment-create-error" role="alert">{{ $message }}</span>@enderror
                        </div>
                ...
                <div class="enrollment-create-actions">
                    <a href="{{ route('admin.enrollments.index') }}" class="btn btn-secondary"><i class="fa-solid fa-xmark" aria-hidden="true"></i> Cancel</a>
