...
<div class="user-page enrollment-edit-page">
    <x-user-page-header
        ...
        subtitle="Update progress, status, and notes for this student’s enrollment."
    ...
    <div class="enrollment-edit-layout">
        <aside class="enrollment-summary" aria-label="Enrollment summary">
            <div class="enrollment-summary-top">
                ...
                <span class="user-status {{ $enrollment->status }}">{{ ucfirst($enrollment->status) }}</span>
            </div>
            <span class="enrollment-summary-kicker">Enrollment #{{ $enrollment->id }}</span>
            <h2>{{ $enrollment->student?->full_name ?? 'Unknown student' }}</h2>
            <p>{{ $enrollment->student?->email ?? 'No email available' }}</p>

            <dl class="enrollment-summary-list">
                <div><dt><i class="fa-solid fa-school" aria-hidden="true"></i> Class</dt><dd>{{ $enrollment->class?->code ?? '—' }}</dd></div>
                <div><dt><i class="fa-solid fa-book" aria-hidden="true"></i> Course</dt><dd>{{ $enrollment->class?->course?->title ?? $enrollment->class?->course?->name ?? '—' }}</dd></div>
                <div><dt><i class="fa-solid fa-calendar" aria-hidden="true"></i> Enrolled</dt><dd>{{ $enrollment->enrolled_at?->format('M d, Y') ?? '—' }}</dd></div>
            </dl>

            <a class="enrollment-summary-link" href="{{ route('admin.students.show', $enrollment->student) }}">
        ...
        <div class="user-panel enrollment-edit-panel">
            <div class="enrollment-edit-panel-head">
                ...
                <span class="enrollment-edit-id">ID {{ $enrollment->id }}</span>
            </div>

            <form action="{{ route('admin.enrollments.update', $enrollment) }}" method="POST" id="enrollmentEditForm" class="enrollment-edit-form">
                @csrf
                @method('PUT')
                ...
                <section class="enrollment-edit-section" aria-labelledby="status-heading">
                    ...
                    <div class="enrollment-edit-field @error('status') has-error @enderror">
                        <label for="status">Status <span aria-hidden="true">*</span></label>
                        <select id="status" name="status" required aria-describedby="status-help status-error" @error('status') aria-invalid="true" @enderror>
                            @foreach(['pending' => 'Pending — awaiting confirmation', 'active' => 'Active — currently enrolled', 'completed' => 'Completed — course finished', 'dropped' => 'Dropped — no longer enrolled'] as $value => $label)
                        ...
                        @error('status')<span id="status-error" class="enrollment-field-error" role="alert">{{ $message }}</span>@enderror
                    </div>
                </section>
                ...
                <section class="enrollment-edit-section" aria-labelledby="grade-heading">
                    ...
                    <div class="enrollment-edit-field @error('final_grade') has-error @enderror">
                        <label for="final_grade">Final grade <span class="enrollment-edit-optional">Optional</span></label>
                        <div class="enrollment-grade-input"><input id="final_grade" type="number" name="final_grade" value="{{ old('final_grade', $enrollment->final_grade) }}" min="0" max="100" step="0.01" placeholder="0.00" aria-describedby="grade-help grade-error" @error('final_grade') aria-invalid="true" @enderror><span>/ 100</span></div>
                        ...
                        @error('final_grade')<span id="grade-error" class="enrollment-field-error" role="alert">{{ $message }}</span>@enderror
                    </div>
                </section>
                ...
                <div class="enrollment-edit-actions">
                    <a href="{{ route('admin.enrollments.show', $enrollment) }}" class="btn btn-secondary"><i class="fa-solid fa-xmark" aria-hidden="true"></i> Cancel</a>
