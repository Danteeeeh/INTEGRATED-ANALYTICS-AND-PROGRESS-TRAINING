    ...
    <main class="auth-frame">
        <section class="brand-panel" aria-label="Bestlink College branding">
            <div class="brand-content">
                <img class="brand-mark" src="{{ asset('images/BCP_LOGO.png') }}" alt="Bestlink College of the Philippines logo">
        ...
        <section class="form-panel">
            <div class="form-wrap">
                ...
                @if ($errors->any())<div class="message error" role="alert"><i class="fa-solid fa-circle-exclamation"></i> {{ $errors->first() }}</div>@endif
                @if (session('status'))<div class="message success" role="status"><i class="fa-solid fa-circle-check"></i> {{ session('status') }}</div>@endif
                <form method="POST" action="{{ route('login') }}" id="loginForm">
                    @csrf
                    <div class="field"><label for="email">Email address</label><div class="input-wrap"><i class="fa-regular fa-envelope" aria-hidden="true"></i><input type="email" id="email" name="email" value="{{ old('email') }}" placeholder="you@example.com" autocomplete="email" required autofocus></div></div>
