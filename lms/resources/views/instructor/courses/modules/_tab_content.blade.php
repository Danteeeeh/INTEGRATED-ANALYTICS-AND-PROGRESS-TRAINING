...
<div class="crud-card" style="margin: 0 24px 24px;">
    <div class="crud-header">
        <h3>Modules ({{ $course->modules->count() ?? 0 }})</h3>
    ...
    <div style="padding: 16px 24px;">
        @if(($course->modules->count() ?? 0) > 0)
            @foreach($course->modules as $module)
                <div class="module-mini-card">
                    ...
                    <div class="module-mini-body">
                        <div class="module-mini-title">#{{ $module->order ?? $loop->iteration }} — {{ $module->title }}</div>
                        <div class="module-mini-meta">
                            <i class="fa-solid fa-book-open"></i> {{ $module->lessons->count() ?? 0 }} lessons
            ...
            <div style="margin-top: 10px; text-align: center;">
                <a href="{{ route('instructor.courses.modules.index', $course) }}" style="color:#2563eb;text-decoration:underline;font-weight:600;font-size:0.84rem;">
                    View All Modules →
                </a>
            </div>
        @else
            <div style="text-align:center;padding:32px;color:#aaa;">
                ...
                No modules yet. <a href="{{ route('instructor.courses.modules.create', $course) }}" style="color:#2563eb;text-decoration:underline;">Create one</a>
            </div>
        @endif
    </div>
</div>
