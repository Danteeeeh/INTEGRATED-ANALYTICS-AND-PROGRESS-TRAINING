        ...
        <div class="user-panel-body">
            <form action="{{ route('instructor.courses.quizzes.update', [$course, $quiz]) }}" method="POST">
                    ...
                    <div class="form-field">
                        ...
                        <select name="class_id">
                            @foreach($classes as $class)
                                <option value="{{ $class->id }}" @selected(old('class_id', $quiz->class_id) == $class->id)>{{ $class->code }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-field full">
                        <div class="checkbox-grid">
                            <label class="checkbox-label">
                                <input type="checkbox" name="shuffle_questions" value="1" @checked(old('shuffle_questions', $quiz->shuffle_questions))> Shuffle questions
                            </label>
                            <label class="checkbox-label">
                                <input type="checkbox" name="shuffle_choices" value="1" @checked(old('shuffle_choices', $quiz->shuffle_choices))> Shuffle choices
                            </label>
                            <label class="checkbox-label">
                                <input type="checkbox" name="review_allowed" value="1" @checked(old('review_allowed', $quiz->review_allowed))> Allow review
                            </label>
                            <label class="checkbox-label">
                                <input type="checkbox" name="show_correct_answers" value="1" @checked(old('show_correct_answers', $quiz->show_correct_answers))> Show correct answers
                            </label>
                        </div>
                    </div>
                ...
                <div class="form-actions user-actions">
                    <a href="{{ route('instructor.courses.quizzes.show', [$course, $quiz]) }}" class="btn btn-secondary"><i class="fa-solid fa-times"></i> Cancel</a>
