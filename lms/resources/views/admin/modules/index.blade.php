    ...
    <div class="crud-card">
        <div class="crud-header">
            ...
            <div style="display: flex; gap: 10px; align-items: center;">
                <a href="{{ route('admin.courses.show', $course) }}" class="btn-add" style="background: linear-gradient(135deg, #64748b 0%, #475569 100%);">
                    <i class="fa-solid fa-arrow-left"></i>
                    Back to Course
                </a>
                <a href="{{ route('admin.courses.modules.create', $course) }}" class="btn-add">
        ...
        @if($modules->count() > 0)
            <form id="reorderForm" method="POST" action="{{ route('admin.courses.modules.reorder', $course) }}">
                @csrf
            </form>
        @endif
        ...
        <table class="crud-table" id="sortable-table">
            ...
            <tbody id="sortable-tbody">
                @forelse($modules as $module)
                    <tr data-id="{{ $module->id }}" style="cursor: move;">
                        <td class="position-cell">
                            <i class="fa-solid fa-grip-vertical" style="color: #94a3b8; margin-right: 8px;"></i>
                            <span class="position-num">{{ $loop->iteration }}</span>
                            <input type="hidden" name="order[]" form="reorderForm" value="{{ $module->id }}">
                        </td>
                        <td>
                        ...
                        <td class="actions-cell">
                            <a href="{{ route('admin.courses.modules.show', [$course, $module]) }}" class="btn-icon btn-view" title="View">
                                <i class="fa-solid fa-eye"></i>
                            </a>
                            <a href="{{ route('admin.courses.modules.lessons.index', [$course, $module]) }}" class="btn-icon btn-edit" title="Manage Lessons" style="background: #0ea5e9;">
                                <i class="fa-solid fa-book-open"></i>
                            </a>
                            <a href="{{ route('admin.courses.modules.edit', [$course, $module]) }}" class="btn-icon btn-edit" title="Edit">
                                <i class="fa-solid fa-pen-to-square"></i>
                            </a>
                            @if($module->status === 'published')
                                <form method="POST" action="{{ route('admin.courses.modules.unpublish', [$course, $module]) }}" class="inline">
                                    @csrf
                            ...
                            @else
                                <form method="POST" action="{{ route('admin.courses.modules.publish', [$course, $module]) }}" class="inline">
                                    @csrf
                            ...
                            @endif
                            <form method="POST" action="{{ route('admin.courses.modules.destroy', [$course, $module]) }}" class="inline">
                                @csrf
                                @method('DELETE')
                ...
                @empty
                    <tr>
                        <td colspan="4" style="text-align:center;padding:24px;color:#aaa;">
                            No modules found for this course. <a href="{{ route('admin.courses.modules.create', $course) }}" class="text-blue-600 hover:underline">Create one</a>
                        </td>
                    </tr>
                @endforelse
            </tbody>
