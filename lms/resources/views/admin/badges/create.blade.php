...
<div class="user-page">
    <x-user-page-header
        title="New Badge"
        subtitle="Define a badge and its award criteria."
        icon="fa-medal"
    >
        <x-slot name="actions">
            ...
            <form action="{{ route('admin.badges.store') }}" method="POST">
                ...
                <div class="form-grid">
                    <div class="form-field full">
                        ...
                        <input type="text" name="name" value="{{ old('name') }}" required placeholder="e.g. Perfect Attendance">
                    ...
                    <div class="form-field">
                        ...
                        <select name="badge_type" required>
                            @foreach(['course' => 'Course', 'competency' => 'Competency', 'achievement' => 'Achievement', 'participation' => 'Participation'] as $value => $label)
                                <option value="{{ $value }}" @selected(old('badge_type', 'achievement') === $value)>{{ $label }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-field">
                        ...
                        <input type="text" name="icon" value="{{ old('icon', 'fa-award') }}" placeholder="e.g. fa-trophy">
                    ...
                    <div class="form-field full">
                        ...
                        <textarea name="criteria_description" rows="3" placeholder="How is this badge earned?">{{ old('criteria_description') }}</textarea>
                        <span class="field-error">{{ $errors->first('criteria_description') }}</span>
                    </div>

                    <div class="form-field full">
                        <label>Description</label>
                        <textarea name="description" rows="3" placeholder="Badge description">{{ old('description') }}</textarea>
