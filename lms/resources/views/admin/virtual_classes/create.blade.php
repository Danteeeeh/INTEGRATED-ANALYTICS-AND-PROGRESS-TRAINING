...
@section('title', 'Create Virtual Class')
@php
    $activeNav = 'courses';
...
@section('content')
    <div class="form-card">
        ...
        <form method="POST" action="{{ route('admin.virtual_classes.store') }}">
            @csrf
            ...
            <div class="form-grid">
                ...
                <div class="form-field full">
                    ...
                    <textarea name="description" rows="3" placeholder="Agenda, topics, notes...">{{ old('description') }}</textarea>
                </div>
            </div>
            ...
            <div class="form-grid">
                <div class="form-field">
                    ...
                    <select name="meeting_provider" required>
                        ...
                        <option value="zoom" {{ old('meeting_provider') == 'zoom' ? 'selected' : '' }}>Zoom</option>
                        <option value="google_meet" {{ old('meeting_provider') == 'google_meet' ? 'selected' : '' }}>Google Meet</option>
                        <option value="microsoft_teams" {{ old('meeting_provider') == 'microsoft_teams' ? 'selected' : '' }}>Microsoft Teams</option>
                        <option value="other" {{ old('meeting_provider') == 'other' ? 'selected' : '' }}>Other</option>
                    </select>
                    <span class="field-error"></span>
                </div>
            ...
            <div class="form-submit">
                <a href="{{ route('admin.virtual_classes.index') }}" class="btn-modal-cancel" style="display: inline-flex; align-items: center; justify-content: center; padding: 11px 60px; border-radius: 8px; font-size: 0.92rem; font-weight: 600; cursor: pointer; transition: background 0.2s; text-decoration: none; margin-right: 10px;">Cancel</a>
