...
<div class="learning-shell">
    {{-- ═══ HERO ═══ --}}
    <x-user-page-header
        title="Good day, {{ auth()->user()->name ?? 'Student' }}"
        subtitle="Pick up where you left off — here's what needs your attention today."
        icon="fa-graduation-cap"
        kicker="Student learning space"
    >
        <x-slot name="meta">
    ...
    <div class="dash-quick">
        <div class="qa-grid">
            <a href="{{ route('student.courses.index') }}" class="qa-card">
            ...
            <a href="{{ route('student.enrollments.index') }}" class="qa-card">
    ...
    {{-- ═══ STAT TILES ═══ --}}
    @php $overall = (float)($stats['overall_progress'] ?? 0); @endphp
    <div class="user-stat-grid">
        ...
        <x-user-stat-card
            ...
            trend="Live"
        >
            <x-slot name="footer"><i class="fa-solid fa-fire" style="color:#fbbf24"></i> <span id="stat-streak-value">{{ $stats['learning_streak'] ?? 0 }}</span>-day streak</x-slot>
        </x-user-stat-card>
        <x-user-stat-card
            label="Average Grade"
    ...
    @endphp
    @if($assessment)
        <section class="student-assessment student-assessment-{{ $assessmentStatus }}" aria-labelledby="student-assessment-title">
            ...
            <div class="student-assessment-body">
                <div class="student-assessment-score">
                    <div class="student-risk-meter" style="--risk-score: {{ $assessment['risk_score'] }}%" role="img" aria-label="Risk score {{ $assessment['risk_score'] }} out of 100">
                        <span><strong>{{ $assessment['risk_score'] }}</strong><small>risk score</small></span>
                    </div>
                    <div>
                        <strong>{{ $assessmentStatus === 'at_risk' ? 'A little support can help' : ($assessmentStatus === 'on_track' ? 'You are building momentum' : 'Let’s build your baseline') }}</strong>
                ...
                <div class="student-assessment-signals" aria-label="Performance signals">
                    @foreach($assessment['signals'] as $signal)
                        <div class="student-signal student-signal-{{ $signal['status'] }}">
                            <span class="student-signal-dot" aria-hidden="true"></span>
                            <div><strong>{{ $signal['label'] }}</strong><span>{{ $signal['value'] }}</span><small>{{ $signal['detail'] }}</small></div>
                        </div>
                    @endforeach
                </div>
                    ...
                    <div>
                        ...
                        <ul class="student-assessment-list">
                            @foreach($assessment['reasons'] as $reason)
                                <li><i class="fa-solid fa-circle-info" aria-hidden="true"></i><span>{{ $reason }}</span></li>
                            @endforeach
                        </ul>
                    </div>
                    <div>
                        ...
                        <div class="student-recommendations">
                            @foreach($assessment['recommendations'] as $recommendation)
                                <div class="student-recommendation">
                                    <span><i class="fa-solid {{ $recommendation['icon'] }}" aria-hidden="true"></i></span>
    ...
    @if($continue && isset($continue['course']))
        <div class="continue-strip">
            <div class="continue-card">
                ...
                <div class="cc-body">
                    ...
                    <h4 class="cc-title">{{ $continue['course']->title }}</h4>
                    <p class="cc-sub">{{ $continue['course']->code ?? '' }} · last accessed {{ $continue['last_accessed'] ? $continue['last_accessed']->diffForHumans() : 'recently' }}</p>
                </div>
    ...
    <div class="dash-grid">
        ...
        <section class="dash-panel">
            <div class="dash-panel-head">
                ...
                <span class="panel-count">{{ count($progressData) }}</span>
            </div>
            <ul class="dash-list">
                @forelse($progressData as $item)
                    <li class="dash-list-item">
                        <a class="dash-list-link" href="{{ ($item['course']->id ?? null) ? route('student.courses.show', $item['course']->id) : '#' }}">
                            <span class="dash-list-icon i-green"><i class="fa-solid fa-book"></i></span>
                            <div class="dash-list-body">
                                <p class="dash-list-title">{{ $item['course']->title }}</p>
                                <p class="dash-list-sub">{{ $item['course']->code ?? '' }}</p>
                            </div>
                            <div class="dash-list-meta">
                                <div class="dash-progress"><span style="width: {{ $item['progress'] }}%"></span></div>
                                <span class="dash-progress-label">{{ round($item['progress']) }}%</span>
                            </div>
                        </a>
                    </li>
                @empty
    ...
    <div class="dash-grid">
            ...
            <div style="padding:14px">
                <div class="achieve-row">
                    ...
                    <div class="achieve-tile">
                        ...
                        <span class="at-count">{{ $stats['total_virtual_classes'] ?? 0 }}</span>
                        <span class="at-label">Virtual Classes</span>
                    </div>
                </div>
                @if(($stats['badges'] ?? collect())->isNotEmpty())
                    <div class="earned-badges">
                        ...
                        <div class="earned-badges-row">
                            @foreach(($stats['badges'] ?? []) as $badgeAward)
                                <span class="earned-badge" title="{{ $badgeAward->badge?->description ?? '' }}">
                                    <i class="fa-solid {{ $badgeAward->badge?->icon ?: 'fa-award' }}"></i>
                                    {{ $badgeAward->badge?->name ?? 'Badge' }}
                                </span>
                            @endforeach
                        </div>
                    </div>
        ...
        <section class="dash-panel">
            <div class="dash-panel-head">
                ...
                <span class="panel-count">{{ $stats['total_announcements'] ?? 0 }}</span>
            </div>
            <ul class="announce-feed">
                @forelse(($stats['announcements'] ?? []) as $announcement)
                    <li class="announce-item">
                        <div class="announce-date">
                            <span class="ad-day">{{ $announcement->publish_at?->format('j') }}</span>
                            <span class="ad-mon">{{ $announcement->publish_at?->format('M') }}</span>
                        </div>
                        <div class="announce-body">
                            <p class="announce-title">{{ $announcement->title }}</p>
                            <p>{{ Str::limit($announcement->body ?? $announcement->content ?? '', 90) }}</p>
                        </div>
                    </li>
                @empty
    ...
    {{-- ═══ RECENT GRADES + VIRTUAL CLASSES ═══ --}}
    <div class="dash-grid">
        <section class="dash-panel">
            <div class="dash-panel-head">
                <h4><i class="fa-solid fa-graduation-cap"></i> Recent grades</h4>
                <span class="panel-count">{{ $stats['total_grades'] ?? 0 }}</span>
            </div>
            <ul class="dash-list">
                @forelse(($stats['recent_grades'] ?? []) as $grade)
                    <li class="dash-list-item">
                        <a class="dash-list-link" href="{{ $grade->item?->class_id ? route('student.classes.show', $grade->item->class_id) : '#' }}">
                            ...
                            <div class="dash-list-meta">
                                <span class="dash-meta-chip {{ ($grade->score_percent ?? 0) >= 75 ? 'm-green' : 'm-rose' }}">{{ number_format($grade->score_percent ?? 0, 1) }}%</span>
                                <span class="dash-list-date">{{ $grade->graded_at?->diffForHumans() }}</span>
                            </div>
                        </a>
                    </li>
                @empty
        ...
        <section class="dash-panel">
            <div class="dash-panel-head">
                ...
                <span class="panel-count">{{ $stats['total_virtual_classes'] ?? 0 }}</span>
            </div>
            <ul class="dash-list">
    ...
    <div class="dash-grid">
        <section class="dash-panel">
            <div class="dash-panel-head">
                ...
                <span class="panel-count">{{ ($stats['recent_quiz_attempts'] ?? collect())->count() }}</span>
            </div>
            <ul class="dash-list">
                @forelse(($stats['recent_quiz_attempts'] ?? []) as $attempt)
                    <li class="dash-list-item">
                        ...
                        <a class="dash-list-link" href="{{ $attemptHref }}">
                            ...
                            <div class="dash-list-body">
                                <p class="dash-list-title">{{ $attempt->quiz?->title ?? 'Quiz' }}</p>
                                <p class="dash-list-sub">{{ $quizCourse?->title ?? '' }} · Attempt {{ $attempt->attempt_number ?? 1 }}</p>
                            </div>
                            <div class="dash-list-meta">
        ...
        <section class="dash-panel">
            <div class="dash-panel-head">
                ...
                <span class="panel-count">{{ ($stats['recent_feedback'] ?? collect())->count() }}</span>
            </div>
            <ul class="dash-list">
                @forelse(($stats['recent_feedback'] ?? []) as $feedback)
                    <li class="dash-list-item">
                        <span class="dash-list-icon i-green"><i class="fa-solid fa-comment-dots"></i></span>
                        <div class="dash-list-body">
                            <p class="dash-list-title">{{ Str::limit($feedback->body ?? 'Feedback', 60) }}</p>
                            <p class="dash-list-sub">{{ $feedback->author?->name ?? 'Instructor' }} · {{ $feedback->created_at?->diffForHumans() }}</p>
                        </div>
                        <div class="dash-list-meta">
                            @if(($feedback->rating ?? 0) > 0)
                                <span class="dash-meta-chip m-amber"><i class="fa-solid fa-star"></i> {{ $feedback->rating }}/5</span>
                            @endif
                        </div>
                    </li>
                @empty
    ...
    <script>
        ...
        async function refreshDashboard() {
            ...
            try {
                await fetch('{{ route('student.dashboard.clear-cache') }}', {
                    method: 'POST',
                    headers: {
        ...
        setInterval(async () => {
            try {
                const response = await fetch('{{ route('student.dashboard.real-time-stats') }}');
        ...
        async function performSearch(query) {
            ...
            try {
                const response = await fetch(`{{ route('student.dashboard.search') }}?query=${encodeURIComponent(query)}&type=${type}`);
                const data = await response.json();
                if (data.success) {
        ...
        function displaySearchResults(results, type) {
            ...
            results.forEach(result => {
                ...
                if (type === 'courses' || (result.title && result.code && !result.due_date)) {
                    href = result.id ? `{{ url('student/courses') }}/${result.id}` : '#';
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            fetch(`{{ route('student.dashboard.analytics') }}?type=grades`)
                .then(r => r.json())
